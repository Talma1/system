...
import { addCouponAction } from '../actions/couponsActions';
import {Coupon} from "../models/coupons";


const AddNewCoupon = () => {
  const fields = [ 'header', 'price', 'company', 'tableActions'];
  const [modalOpen, setModalOpen] = useState(false)
  const [fieldsState, setFieldsState] = useState(new Coupon());
  ...
  return (
    ...
    >
      <Modal.Header>Add New Coupon</Modal.Header>
      <Modal.Content>
        <Modal.Description>
          <Grid stackable>
            <Grid.Row columns={2}>
